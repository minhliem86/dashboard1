			...
			<div class="col-sm-6">
				<article class="each_service">
					...
					<div class="content_service_wrap">
						<h3 class="title"><a href="{{URL::route('user_service_slug', array($item->slug))}}">{{$item->title}}</a></h3>
						<p class="text_service">{{Str::words($item->content,20)}}</p>
						<a href="{{URL::route('user_service_slug', array($item->slug))}}" class="">See more</a>
					</div>
				</article>
			</div>
			@endforeach
